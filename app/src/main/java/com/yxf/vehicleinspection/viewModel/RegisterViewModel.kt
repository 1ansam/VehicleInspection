...
class RegisterViewModel(val registerRepository: RegisterRepository) : ViewModel() {
    ...
    fun getAjJyxm(jyxm : RecyclerView): List<String> {
        val list = ArrayList<String>()
        for (index in 0 until jyxm.adapter!!.itemCount) {
    ...
    fun getHjJyxmString(jyxm : RecyclerView): String {
        var hjJyxmString = ""
        for (index in 0 until jyxm.adapter!!.itemCount) {
            val holder = jyxm.findViewHolderForAdapterPosition(index)
            val checkbox = holder?.itemView?.findViewById<CheckBox>(R.id.cbJyxm)
    ...
    fun getValueMap(textList : RecyclerView): MutableMap<String, String> {
        val map = mutableMapOf<String, String>()
        for (index in 0 until textList.adapter!!.itemCount) {
