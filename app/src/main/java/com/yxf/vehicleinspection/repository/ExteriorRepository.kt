...
import com.yxf.vehicleinspection.bean.request.ArtificialProjectR016Request
...
class ExteriorRepository {
    ...
    fun getSelectItemData(
        ...
        Ajywlb: String,
        Hjywlb: String,
    ): LiveData<List<ArtificialProjectR016Response>> {
        val liveData = MutableLiveData<List<ArtificialProjectR016Response>>()
        val call = RetrofitService.create(QueryService::class.java).query(
            ApiStatic.QUERY_ARTIFICIAL_PROJECT,
            IpHelper.getIpAddress(),
            JsonDataHelper.getJsonData(ArtificialProjectR016Request(Lsh, Jyxm, Ajywlb, Hjywlb))
        )
        call.enqueue(object : Callback<ResponseBody>{
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful){
                    ...
                    if (commonResponse.Code == "1"){
                        val artificialProjectList = ArrayList<ArtificialProjectR016Response>()
                        for (element in commonResponse.Body) {
                            val bodyJson = GsonSingleton.getGson().toJson(element)
                            artificialProjectList.add(GsonSingleton.getGson()
                                .fromJson(bodyJson, ArtificialProjectR016Response::class.java))
                        }
                        liveData.value = artificialProjectList
                    }else{
